...
- **DSD Bit Order Handling**: Normalizes DSD byte ordering per-source (`DsdBitOrder::Lsb` / `DsdBitOrder::Msb`) with a global configurable override. Per-device byte-order, bit-order, byte-reverse, and subslot overrides are now driven from a unified quirk database and synced to the engine before playback; UAC2 alt-settings are probed for DSD/DoP capability before stream start
- **Real-Time Processing Chain**: Implements a complete DSP chain including volume control, 10-band graphic equalizer with preamp, spatial/time FX, convolution reverb (impulse-response convolver), dynamics processing (compressor/limiter), playback speed control, and crossfading (selectable `CrossfadeCurve`)
...
- **Compact Home Widget**: 2×2 `CompactWidgetProvider` widget rendering current-track text and transport controls, backed by its own compact-specific preferences and settings tab. The mini player widget gained a semi-transparent scrim overlay whose visibility tracks album art presence. Widget settings support swipe gestures between tabs with animated transitions.
- **Milestone Streaks & Emerald Tier**: Day-streak tracking records consecutive listening days with a flame-icon popup and optional snooze. A unique-artist-count milestone tracks distinct artists over the listener's lifetime. An emerald tier joins the existing five tiers, and milestones are grouped by category with collapsible sections. `MilestoneService` was refactored with category-based current-value tracking.
- **Audio Convolver (IR Reverb)**: A direct-time-domain convolver processes impulse-response (IR) WAV files for convolution reverb, fed by an offline IR loader and integrated into the audio pipeline via the equalizer service. Persistent `ConvolverSettings` store enable/disable, dry/wet mix, and loaded IR; a convolver section on the equalizer screen drives it. Rust public API: `convolver_enable`, `convolver_mix`, `convolver_load_ir`, `convolver_clear`.
- **Crossfade Engine**: The Rust audio engine crossfades between tracks using a `CrossfadeCurve` enum. Pending crossfade configuration is stored in atomics and survives engine recreation, applying on the next audio state update. A Dart FFI API exposes pending crossfade and DSD override options.
- **DSD Transport Overrides**: Per-device DSD byte-order and subslot overrides are applied through the unified quirk database (`sub_slot_size`, `bit_order`, `byte_reverse`), synced to the Rust engine before playback. 24-bit DSD over USB DoP uses corrected bits-per-frame; UAC2 alt-settings are probed for DSD/DoP capability before stream start.
- **Removable Storage Scanning**: SD card and USB drive folders are scannable via Android SAF with per-volume MediaStore support. `FolderEntity` gained `isRemovable`, `mediaStoreVolume`, and `volumeState`; an external status label appears on the root folder card, USB/removable status shows inline during deep scans, and unmount events fall back to an instant SAF scan. The SAF tree walk uses `contentResolver.query` per directory.
- **Bluetooth Management**: A Bluetooth settings screen shows A2DP codec info and battery level for connected devices, backed by a service layer with device and codec DTOs. Low-latency mode selects the Rust Oboe engine for Bluetooth, pause-on-disconnect auto-pauses when Bluetooth drops, and reconnect resume restarts playback on reconnect. A Bluetooth connect permission supports Android 12+.
- **Metadata Editor Safety**: Tag writes are verified with typed outcomes and metadata is validated before saving. The original file is copied to a temp path before writing for safe rollback, an SAF fallback handles scoped-storage tag writes, and write URI permission is requested and persisted.
- **Milestone Tracking**: Achievement system tracking songs played (100/500/1000) and listening time (10/50 hours). `MilestoneService` with accumulated listen time, milestone dialogs from app shell, and persistent "shown" state per milestone. Collection view with per-tier accent colors (bronze/silver/gold/sapphire/amethyst/emerald), hero icons, and "next milestone" hint line.
...
- **Crossfader (`crossfader.rs`)**: Handles the smooth blending between songs, so there is no silence when one track ends and the next begins. Supports selectable curves via `CrossfadeCurve`, with pending configuration held in atomics so it survives engine recreation and applies on the next audio state update.
- **Convolver (`convolver/`)**: Direct-time-domain convolution reverb. Loads impulse-response (IR) WAV files via an offline loader and streams them through a convolver with dry/wet mix. Integrated through the equalizer service with persistent `ConvolverSettings`; exposed to Dart via `convolver_enable`, `convolver_mix`, `convolver_load_ir`, and `convolver_clear`.
...
- **MiniPlayerWidgetProvider (Kotlin)**: Native `AppWidgetProvider` rendering album art, song info, progress bar, and transport controls. Shows idle "Tap to open" state when the app is killed, active state during playback. A semi-transparent scrim overlay's visibility matches album art presence.
- **CompactWidgetProvider (Kotlin)**: Native 2×2 compact widget rendering current-track text and transport controls, driven by its own compact-specific preferences.
...
- **Widget Settings Screen (`widget_settings_screen.dart`)**: UI for customizing background opacity, accent color, and visible content (album art, artist name), with a dedicated Compact tab and swipe gestures between tabs with animated transitions.
...
- **Removable storage**: Folders on SD cards and USB drives are scanned via Android SAF with per-volume MediaStore support. `FolderEntity` carries `isRemovable`, `mediaStoreVolume`, and `volumeState` (resolved when the folder is added); unmount events fall back to an instant SAF scan, and the SAF tree walk uses `contentResolver.query` per directory for faster traversal
